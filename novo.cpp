...
void atualiza_log(char palavra[50], int codigo);
...
void atualiza_log(char palavra[50], int codigo)
{
    ...
    {
        log=fopen("log.txt","w+"); //Cria o arquivo se ele não existe.
        fclose(log);
    }
    log=fopen("log.txt","a+");

    strftime (hora,80,"%d/%m/%y - %H:%M:%S - ",timeinfo); //Pega a hora.

    switch(codigo){

        case 1://Erro ao abrir o arquivo de entrada.
            fputs(hora,log);
            fputs("Erro ao abrir arquivo de entrada\n",log);
            break;
        case 2: //Usuário X foi inserido.
            fputs(hora,log);
            fputs("O usuário ",log);
            fputs(palavra,log);
            fputs(" foi inserido.\n",log);
            break;
        case 3://Local X foi inserido na lista
            fputs(hora,log);
            fputs("O local ",log);
            fputs(palavra,log);
            fputs(" foi inserido\n",log);
            break;
        case 4://Fon inserido o usuário X na lista de retweets
            fputs(hora,log);
            fputs("O usuário ",log);
            fputs(palavra,log);
            fputs(" foi inserido na lista de retweets.\n",log);
            break;
        case 5://Atualizou lista mentions.
            fputs(hora,log);
            fputs("O usuário ",log);
            fputs(palavra,log);
            fputs(" foi inserido na lista de mentions.\n",log);
            break;
        case 6://Atualizou a lista de hashtags
            fputs(hora,log);
            fputs("A palavra ",log);
            fputs(palavra,log);
            fputs(" foi inserida na lista de hashtags.\n",log);
            break;
        case 7://
...
GERAL *case_1(GERAL *novo)
{
    ...
    char nome_usuario[50], palavra[140], nome_hashtag[140], nome_RT[140], nome_mention[140], local[140], auxiliar[140],string[140],nada[0];
    Inf_tweet ult_tweet;
    int i=0,sair=0,pos_atual=0,pos_fim=0, diferente=0,riso,acabou=0,j=0, option, total_tweets=0,cont, flag=0;
    system("cls");
    ...
    if(arq = fopen(nome_usuario,"r+"))
    {
        ...
        atualiza_log(nome_usuario,2); //"USUARIO X FOI INSERIDO"
        while(strcmp(aux_user->nome,nome_usuario))   //CARREGA A STRUCT DO USUÁRIO A TER INFORMAÇÕES PREENCHIDAS
            aux_user = aux_user->prox;
        fscanf(arq,"%s",palavra); //Lê a virgula
        flag=0;
        fscanf(arq,"%s",palavra); //Lê local do usuário.
